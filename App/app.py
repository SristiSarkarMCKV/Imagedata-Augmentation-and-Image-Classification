...
def inject_custom_styles(bg_url):
    ...
    css_template = """
        ...
        .stApp {
            background-image: linear-gradient(rgba(15, 23, 42, 0.70), rgba(15, 23, 42, 0.70)), url("BG_URL_PLACEHOLDER");
    ...
    """
    final_css = css_template.replace("BG_URL_PLACEHOLDER", bg_url)
    st.markdown(final_css, unsafe_allow_html=True)


def render_mermaid(code):
    """Renders Mermaid.js diagrams using an iframe component."""
    html = f"""
    ...
    """
    calculated_height = max(300, len(code.split('\n')) * 35)
    components.html(html, height=calculated_height)
...
if nav_choice == "🏠 Home":
    ...
    # Workflow Pipeline Breakdown Cards
...
def render_mermaid(code):
    ...
    """
    # Calculate height based on number of lines in code to avoid cutoffs
    calculated_height = max(300, len(code.split('\n')) * 35)
    components.html(html, height=calculated_height)
...
if nav_choice == "🏠 Home":
    ...
    # Diagrammatic Explanation (New Section)
    st.write("")
    st.markdown("#### 📊 Visual Workflow Diagram")
    
    # Mermaid diagram defining the flow
    mermaid_code = """
    %%{init: {'theme': 'base', 'themeVariables': { 'primaryColor': '#EDF2F7', 'edgeLabelBackground':'#ffffff', 'tertiaryColor': '#fff'}}}%%
    graph TD
        Start(📥 User Uploads Image) --> Read[📸 Read Image & Convert to RGB]
...
# =========================================================
    ...
    # ... (Rest of the prediction page code remains unchanged)
    if st.session_state.page == 'upload':
        # ... (upload code)
        st.markdown(
            "<p class='sub-text'>"
...
# =========================================================
    ...
    st.markdown("### ℹ️ About the Model & Technology")
    # ... (Rest of the about page code remains unchanged)
    st.markdown(
        """
        #### 🤖 Model Architecture: ResNet-50
        This system leverages **ResNet-50**, a residual deep learning neural network architecture with 50 layers. 
